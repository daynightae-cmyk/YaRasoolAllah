...
| Known unstable gate (observed 2026-09-25) | `verifyAudioFailureState` in `scripts/smoke-visual-golden.mjs` intermittently times out on `quran-1440-rtl` / `audio-1440-rtl` "stream error": the synthetic `error` event can be dispatched before React attaches the handler, and the assertion has no re-dispatch. Observed twice, both times passing on an immediate retry, and unrelated to application code. Needs a deterministic dispatch/retry rather than a re-run. |
...
1. Make the audio failure gate deterministic (`verifyAudioFailureState` dispatches before React attaches the handler; see the known unstable gate row).
2. Confirm the original characters of the one remaining reviewed encoding allowance in `client/src/components/BabAlsamaa/BabAlsamaa.tsx:75` (hadith text, letter vs diacritic) with a scholar; it is deliberately not reconstructed by guessing.
3. Persistence: production `DATABASE_URL` required; memory fallback is not production.
4. Multilingual closure: separate UI-language support from content-translation availability (AR/EN/FR/UR).
