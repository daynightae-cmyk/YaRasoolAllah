...
export function isArabicOnly(declaration: RouteContentDeclaration): boolean {
  return (
    declaration.kind === "knowledge" &&
    declaration.contentLanguages.length === 1 &&
    declaration.contentLanguages[0] === "ar"
  );
}
...
export function contentLanguageNotice(
  ...
  if (declaration.kind !== "knowledge") return null;
  if (declaration.contentLanguages.includes(uiLang as ContentLanguage)) return null;
  ...
  return {
    textEn:
      available.length === 0
        ...
        : `The interface is in English. The recorded material is in ${named}; it is not an English edition.`,
    textAr:
      available.length === 0
        ? "الواجهة بالإنجليزية، لكن هذا المحتوى مسجَّل بالعربية فقط ولم يُترجم."
        : `الواجهة بالإنجليزية، والمادة المسجَّلة بـ${named}، وليست نسخة إنجليزية.`,
    available,
