...
export function evaluateResourceUsage(
  ...
  // full_text is a distinct legal permission: it is never granted by
  // allowedUsages alone. Only a cleared rights decision with an explicit
  // permission record may open full text.
  if (usage === "full_text") {
    if (rights.decision !== "cleared") {
      return { allowed: false, reason: "عرض النص الكامل يتطلب قرار حقوق cleared صريحًا." };
    }
    if (!resource.allowedUsages.includes("full_text")) {
      return { allowed: false, reason: "النص الكامل غير مسجل ضمن الاستخدامات المسموح بها." };
    }
  }
